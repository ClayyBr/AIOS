...
async function buscar(ingrediente) {
    ...
    try {
        ...
        const produtos = await page.evaluate(() => {
            ...
            const productCards = document.querySelectorAll('[class*="product"], [class*="Product"], [class*="item"], [class*="shelf"], [data-testid*="product"]');
            productCards.forEach((card) => {
                // Nome do produto
                const nomeEl = card.querySelector('h2, h3, a[href*="/produto/"], [class*="name"], [class*="Name"], [class*="title"], [class*="Title"]');
                ...
                try {
                    ...
                    if (Array.isArray(products)) {
                        for (const prod of products) {
                            ...
                            const preco = prod.priceRange?.sellingPrice?.lowPrice ||
