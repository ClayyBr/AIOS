...
): TE.TaskEither<AppError, SuccessStoryDocument> =>
  pipe(
    E.tryCatch(
      () => successStorySchema.parse(dto),
      (reason) => ({
        type: 'ValidationError',
        message: 'Invalid success story data provided.',
        details: reason,
      })
    ),
    TE.fromEither,
    TE.chainW((validatedData) =>
      tryCatchTE(() => new SuccessStoryModel(validatedData).save(), 'DatabaseError')
    )
  );
...
export const getSuccessStories = (
  ...
  tryCatchTE(
    () =>
      SuccessStoryModel.find(filterFeatured ? { isFeatured: true } : {})
        .sort({ createdAt: -1 })
        .exec(),
    'DatabaseError'
  );
