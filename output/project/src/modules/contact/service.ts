...
export const submitContactForm = (
  dto: CreateContactFormDTO
): TE.TaskEither<AppError, ContactFormDocument> =>
  pipe(
    E.tryCatch(
      () => contactFormSchema.parse(dto),
      (reason) => ({
        type: 'ValidationError',
        message: 'Invalid contact form data provided.',
        details: reason,
      })
    ),
    TE.fromEither,
    TE.chainW((validatedData) =>
      tryCatchTE(() => new ContactFormModel(validatedData).save(), 'DatabaseError')
    )
...
export const getContactForms = (): TE.TaskEither<AppError, ContactFormDocument[]> =>
  tryCatchTE(() => ContactFormModel.find({}).sort({ createdAt: -1 }).exec(), 'DatabaseError');
