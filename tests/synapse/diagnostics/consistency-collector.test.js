...
describe('collectConsistencyMetrics', () => {
  ...
  test('bracket check FAIL for unknown bracket', () => {
    ...
    writeJson(
      ...
      buildHookMetrics({ bracket: 'INVALID' })
    );
    const result = collectConsistencyMetrics(tmpDir);
    const bracket = result.checks.find((c) => c.name === 'bracket');
  ...
  test('timestamp check PASS when gap < 30s', () => {
    ...
    writeJson(
      path.join(tmpDir, '.synapse', 'metrics', 'uap-metrics.json'),
      buildUapMetrics({ timestamp: now })
    );
    writeJson(
      path.join(tmpDir, '.synapse', 'metrics', 'hook-metrics.json'),
      buildHookMetrics({ timestamp: now })
    );
    const result = collectConsistencyMetrics(tmpDir);
  ...
  test('quality check PASS for full quality + layers loaded', () => {
    writeJson(
      path.join(tmpDir, '.synapse', 'metrics', 'uap-metrics.json'),
      buildUapMetrics({ quality: 'full' })
    );
    writeJson(
      path.join(tmpDir, '.synapse', 'metrics', 'hook-metrics.json'),
      buildHookMetrics({ layersLoaded: 5 })
    );
    const result = collectConsistencyMetrics(tmpDir);
