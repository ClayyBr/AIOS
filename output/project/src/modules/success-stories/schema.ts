...
export const successStorySchema = z.object({
  ...
  storyTitle: z.string().min(5, 'Story title must be at least 5 characters.').max(200, 'Story title too long.'),
  storyContent: z.string().min(50, 'Story content must be at least 50 characters.').max(2000, 'Story content too long.'),
...
export const createSuccessStoryDTOSchema = successStorySchema.omit({ createdAt: true, updatedAt: true });
export type CreateSuccessStoryDTO = z.infer<typeof createSuccessStoryDTOSchema>;

export const updateSuccessStoryDTOSchema = successStorySchema.omit({ createdAt: true, updatedAt: true }).partial();
