 ...
 */
function gerarCSV(resultados, dataColeta) {
    const BOM = "\uFEFF";
...
// ============================================================================
server.tool("buscar_precos_ingredientes", "Pesquisa preços de ingredientes em supermercados atacadistas da região do Vale do Paraíba — SP. " +
    ...
    ingredientes: z
        ...
        .describe("Lista de ingredientes a pesquisar. " +
        "Exemplo: ['arroz 5kg', 'feijão carioca 1kg', 'óleo de soja 900ml']"),
    mercados: z
        .array(z.enum(MERCADOS_DISPONIVEIS))
        .optional()
        .describe("Mercados específicos para consultar (opcional — default: todos). " +
    ...
    const buscasPorMercado = mercadosSelecionados.map(async (mercadoKey) => {
        ...
        for (const ingrediente of ingredientes) {
            try {
                ...
                console.error(`[${scraper.nome}] Erro ao buscar "${ingrediente}":`, error instanceof Error ? error.message : error);
            }
...
async function main() {
    ...
    console.error(`Mercados disponíveis: ${Object.values(SCRAPERS).map((s) => s.nome).join(", ")}`);
}
main().catch((error) => {
