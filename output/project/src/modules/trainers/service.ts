  ...
  pipe(
    E.tryCatch(
      ...
      (reason) => ({
        ...
        message: 'Invalid trainer data provided.',
        details: reason,
      })
    ),
    TE.fromEither,
    TE.chainW((validatedData) =>
      tryCatchTE(() => new TrainerModel(validatedData).save(), 'DatabaseError')
    )
  );

export const getTrainers = (): TE.TaskEither<AppError, TrainerDocument[]> =>
  tryCatchTE(() => TrainerModel.find({ isActive: true }).sort({ name: 1 }).exec(), 'DatabaseError');
...
): TE.TaskEither<AppError, TrainerDocument> =>
  pipe(
    E.tryCatch(
