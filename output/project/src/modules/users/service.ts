...
export const createUser = (dto: CreateUserDTO): TE.TaskEither<CreateUserError, UserDocument> =>
  pipe(
    E.tryCatch(
      ...
      (reason) => ({ type: 'ValidationError', message: 'Invalid user data provided.', details: reason })
    ),
    TE.fromEither,
      ...
      tryCatchTE(
        async () => {
          ...
          if (existingUser) {
            ...
            return Promise.reject({ type: 'DuplicateEmailError', message: 'Email already registered.' });
          }
          const newUser = new UserModel(validatedData);
          return newUser.save();
        },
        (error) => {
          if (typeof error === 'object' && error !== null && 'type' in error && error.type === 'DuplicateEmailError') {
