...
export const trainerSchema = z.object({
  name: z.string().min(2, 'Trainer name must be at least 2 characters.').max(100, 'Trainer name too long.'),
  specialty: z.string().min(3, 'Specialty must be at least 3 characters.').max(100, 'Specialty too long.'),
...
export const updateTrainerDTOSchema = trainerSchema.omit({ createdAt: true, updatedAt: true }).partial();
