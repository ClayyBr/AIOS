    ...
    try {
        ...
        const produtos = await page.evaluate(() => {
            ...
            produtoLinks.forEach((linkEl) => {
                ...
                if (!nome || nome.length < 5 || nomesProcessados.has(nome)) return;
            ...
            if (items.length === 0) {
                ...
                const textos: { texto: string; rect: DOMRect | null }[] = [];
...
export async function closeBrowserSpani(): Promise<void> {
    if (browserInstance) {
...
const spaniScraper: ScraperModule = {
    nome: MERCADO,
    buscar,
};
