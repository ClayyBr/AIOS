...
exports.GoogleSheetsService = exports.sheetsService = void 0;
...
class GoogleSheetsService {
    ...
    init() {
        try {
            if (!env_1.env.GOOGLE_SHEETS_CREDENTIALS) {
                logger_1.logger.warn('⚠️ GOOGLE_SHEETS_CREDENTIALS not configured — SheetsService disabled');
                return;
            }
            if (!env_1.env.SHEETS_CARDAPIO_ID) {
                logger_1.logger.warn('⚠️ SHEETS_CARDAPIO_ID not configured — SheetsService disabled');
                return;
            }
            this.spreadsheetId = env_1.env.SHEETS_CARDAPIO_ID;
            // Parse coordinates (supports both raw JSON or base64 encoded JSON to avoid escaping issues in env vars)
            let credentialsJson;
            try {
                const decoded = Buffer.from(env_1.env.GOOGLE_SHEETS_CREDENTIALS, 'base64').toString('utf8');
                credentialsJson = JSON.parse(decoded.includes('{') ? decoded : env_1.env.GOOGLE_SHEETS_CREDENTIALS);
            }
            catch {
                // Fallback to direct raw JSON parsing
                credentialsJson = JSON.parse(env_1.env.GOOGLE_SHEETS_CREDENTIALS);
            }
            ...
            logger_1.logger.info('📊 GoogleSheetsService initialized');
        }
        catch (error) {
            logger_1.logger.error({ error: error instanceof Error ? error.message : String(error) }, '❌ Failed to initialize GoogleSheetsService');
    ...
    async getRestaurantData() {
        ...
        try {
            ...
            logger_1.logger.error({ error: error instanceof Error ? error.message : String(error) }, 'Failed to fetch sheets data');
    ...
    async fetchFromSheets() {
        ...
        logger_1.logger.info({ itemsCount: items.length, promosCount: promos.length }, '🔄 Menu data refreshed from Google Sheets');
        return {
            items,
            promos,
            formattedMenu,
     ...
     */
    formatPromosText(promos) {
        if (promos.length === 0)
            return 'Nenhuma promoção ativa hoje.';
        let text = '';
        for (const promo of promos) {
            text += `- 🌟 ${promo.name} | Por apenas: ${promo.price}\n`;
            if (promo.description)
                text += `  Detalhes: ${promo.description}\n`;
            if (promo.rules)
                text += `  Regra: ${promo.rules}\n`;
        }
        return text.trim();
    }
    getEmptyData(msg = 'Aguardando sincronização do cardápio.') {
        return {
            items: [],
            promos: [],
            lastUpdate: new Date(),
...
// Singleton
exports.sheetsService = new GoogleSheetsService();
